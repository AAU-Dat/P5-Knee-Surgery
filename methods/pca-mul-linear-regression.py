...
def handle_model(target):
    ...
    # Grid search
    grid_search = GridSearchCV(mul_reg_model, param_grid, cv=5, scoring='neg_root_mean_squared_error', n_jobs=1, verbose=3)
    ...
    best_model = Pipeline([
        ...
        ('mul_linear_reg', linear_model.LinearRegression(n_jobs=1))
    ])
    ...
    train_evaluation = evaluate(y_train, predictions_train)
    test_evaluation = evaluate(y_test, predictions_test)
